    ...
    *,
    ...
    def _create_until_full(
        ...
        *,
        ...
        leftover_booting: list[tuple[str, str, str]] | None = None,
    ) -> list[tuple[str, str]]:
        """Create missing pods, re-checking the live account count before each create.

        ``target`` = pods to create. ``wait_target`` = healthy pods required before
        return (default: all). Still-booting pods go into ``leftover_booting`` so a
        background fill can finish waiting — Pathé can create 8 while only blocking
        on the first healthy GPU.
        """
        target = max(1, min(int(target), account_cap, MAX_SHTETL_PODS))
        wait_for = (
            target
            if wait_target is None
            else max(1, min(int(wait_target), target))
        )
        slot = 0
        last_err: Exception | None = None
        out = list(ready_now)
        ...
        def _wait_one(item: tuple[str, str, str]) -> tuple[str, str] | None:
            ...
            try:
                wait_healthy(
                    base,
                    pod_id=pid,
                    on_status=_gated_status,
                    timeout_sec=1200.0,
                )
                _gated_status(f"pod ready · {name}")
                return (pid, base)
            except (TimeoutError, RuntimeError) as e:
            ...
            if pending:
                ...
                if status_cb:
                    status_cb(
                        ...
                        f"(need {wait_for} ready, ~5–15 min)…"
                    )
                ...
                try:
                    futs = {
                        pool.submit(_wait_one, item): item for item in pending
                    }
                    for fut in concurrent.futures.as_completed(futs):
                        ...
                        if len(out) >= wait_for:
                            still = [
                                item
                                for other, item in futs.items()
                                if other is not fut and not other.done()
                            ]
                            if leftover_booting is not None:
                                leftover_booting.clear()
                    ...
                    pool.shutdown(wait=not early, cancel_futures=False)
                pending = []
    ...
    if len(ready) < n:
        ...
        if create_target > len(ready) and (
            ...
            ready = _create_until_full(
                ...
                wait_target=wait_for,
                leftover_booting=leftover if soft else None,
            )
        if soft and ready:
            # #region agent log
            _dbg(
