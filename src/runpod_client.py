...
_probe_push_lock = threading.Lock()
...
def _local_clip_probe_path():
    from config import OUTPUT_DIR, ROOT

    probe = OUTPUT_DIR / "clip_ft" / "probe.pt"
    if probe.is_file():
        return probe
...
def push_clip_probe_to_pods(
    *,
    on_status: OnStatus | None = None,
    urls: list[str] | None = None,
    force: bool = False,
) -> dict[str, Any]:
    """Install clip_ft/probe.pt on shtetl pods (/clip_probe, else sync_push).

    Skips pods that already received the current probe bytes (unless ``force``).
    Pass ``urls`` to limit to the active scrape pool; otherwise all RUNNING pods.
    """
    import base64
    import hashlib
    ...
    probe = _local_clip_probe_path()
    if probe is None:
        return {"ok": False, "error": "probe_missing", "pushed": 0, "skipped": 0}

    raw = probe.read_bytes()
    probe_sha = hashlib.sha256(raw).hexdigest()
    b64 = base64.b64encode(raw).decode("ascii")
    files = _local_worker_files_for_push()
    scoring = ROOT / "src" / "shtetl_core" / "scoring.py"
    if scoring.is_file():
        files["shtetl_core/scoring.py"] = scoring.read_text(encoding="utf-8")
    entry = ROOT / "runpod_worker" / "entry.py"
    if entry.is_file():
        files["entry.py"] = entry.read_text(encoding="utf-8")

    targets: list[tuple[str, str]] = []  # (base, name)
    if urls:
        for u in urls:
            base = (u or "").rstrip("/")
            if not base:
                continue
            pid = pod_id_from_proxy_url(base) or base.split("//")[-1][:12]
            targets.append((base, pid))
    else:
        for pod in find_shtetl_pods():
            pid = pod.get("id") or ""
            if not pid:
                continue
            targets.append(
                (pod_proxy_url(pid).rstrip("/"), pod.get("name") or pid[:12])
            )
    ...
    for base, name in targets:
        row: dict[str, Any] = {"id": str(name)[:16], "name": name, "base": base}
        if not force:
            with _probe_push_lock:
                if _probe_pushed.get(base) == probe_sha:
                    skipped += 1
                    row["skipped"] = True
                    results.append(row)
                    continue
        try:
            # Prefer dedicated endpoint (avoids fragile sync_push on some images).
            r = requests.post(
                f"{base}/clip_probe",
                json={"b64": b64},
                timeout=120,
            ...
            if not ok and r.status_code == 404:
                r = requests.post(
                    ...
                    json={
                        "files": {
                            k: files[k]
                            for k in ("shtetl_core/scoring.py", "entry.py")
                            if k in files
                        },
                        "files_b64": {"clip_ft/probe.pt": b64},
                    },
                    timeout=180,
                )
                row["http"] = r.status_code
            ...
            if ok:
                pushed += 1
                with _probe_push_lock:
                    _probe_pushed[base] = probe_sha
                if on_status:
                    on_status(f"CLIP probe → {name}")
            elif on_status:
                on_status(f"CLIP probe push {name}: HTTP {r.status_code}")
        except Exception as e:
            row["error"] = str(e)[:160]
            if on_status:
                on_status(f"CLIP probe push {name}: {e}"[:120])
        results.append(row)
    return {
        "ok": pushed > 0 or skipped > 0 or not targets,
        "pushed": pushed,
        "skipped": skipped,
        "probe_sha": probe_sha[:12],
        "results": results,
    }
...
def refresh_pod_pool(
    *,
    ...
    with _refresh_lock:
        ...
        with _pool_lock:
            have = list(_POD_POOL)
        # Only skip when already at target size. Under-filled pools must call
        # ensure_pods or we stay stuck at 1/8 forever (bg async fill used force=False).
        if (
            not force
            and have
            and len(have) >= n
...
def kick_maintain_pod_pool(
    *,
    target: int = 1,
    on_status: OnStatus | None = None,
) -> list[str]:
    """Non-blocking pool heal. Returns the current pool immediately.

    Pathé's claim/result loop used to call ``maintain_pod_pool`` inline; each
    pass probes every pod (multi-second timeouts) and froze completed/hits
    updates for minutes while workers were still running.
    """
    global _maintain_bg_active
    start = False
    with _maintain_bg_lock:
        if not _maintain_bg_active:
            _maintain_bg_active = True
            start = True
    ...
    if start:
        ...
        def _bg() -> None:
            global _maintain_bg_active
            try:
                maintain_pod_pool(target=target, on_status=on_status)
            except Exception as e:
                print(f"[shtetl] bg maintain failed: {e}"[:160], flush=True)
            finally:
                with _maintain_bg_lock:
                    _maintain_bg_active = False

        threading.Thread(
            target=_bg, daemon=True, name="pod-pool-maintain"
        ).start()
    return get_pod_pool()
...
def maintain_pod_pool(
    *,
    ...
    with _pool_lock:
        pool = list(_POD_POOL)
    # CRITICAL: never return early on empty in-memory pool. Healthy GraphQL pods
    # often still exist after the last pool URL was dropped as "dead"; skipping
    # orphan adoption left scrape at 0 workers while GPUs sat idle (death spiral).
    if not pool:
        try:
            adopted: list[str] = []
            for p in find_shtetl_pods():
                ...
                if len(adopted) >= want:
                    break
            if adopted:
                set_pod_pool(adopted)
                pool = list(adopted)
                if on_status:
                    on_status(
                        f"self-heal: adopted {len(adopted)} healthy GPU(s) "
                        f"into empty pool"
                    )
        except Exception:
            pass
        if not pool:
            # Do not block here — Pathé's claim/result loop calls this every ~3s.
            if on_status:
                on_status(
                    "self-heal: GPU pool empty — creating pods in background…"
                )
        ...
        for p in find_shtetl_pods():
            ...
            if kind in ("dead", "broken", "warming", "unknown"):
                with _heal_lock:
                    ...
                    started = float(_warming_since.get(base) or now)
...
def _kick_async_pool_fill(want: int) -> None:
    """Start at most one background ensure_pods so the scrape loop stays free."""
    global _pool_fill_active
    with _pool_fill_lock:
        if _pool_fill_active:
            return
        _pool_fill_active = True
    ...
    def _bg() -> None:
        ...
        try:
            ...
            print(f"[shtetl] async pod fill failed: {e}"[:160], flush=True)
        finally:
            with _pool_fill_lock:
                _pool_fill_active = False

    threading.Thread(target=_bg, daemon=True, name="pod-pool-async-fill").start()


def _push_handlers_best_effort(urls: list[str]) -> None:
    """Push local still-handler + CLIP probe to pods that have not received them yet."""
    for u in urls or []:
        try:
            ...
            pass
    try:
        push_clip_probe_to_pods(urls=list(urls or []), on_status=None)
    except Exception:
        pass
...
def pathe_stack_limit() -> int:
    ...
    with _pathe_stack_lock:
        return max(floor, min(_pathe_stack_limit, mx))
        ...
        if ok:
            ...
            if overload:
                if _pathe_stack_limit > floor:
                    _pathe_stack_limit = max(floor, _pathe_stack_limit - 1)
                _pathe_stack_fail_streak = 0
            else:
                _pathe_stack_fail_streak += 1
                if (
                    _pathe_stack_fail_streak >= _PATHE_FAIL_STRIKES
                    and _pathe_stack_limit > floor
                ):
                    _pathe_stack_limit = max(floor, _pathe_stack_limit - 1)
                    _pathe_stack_fail_streak = 0
        if _pathe_stack_limit < floor:
...
def _process_video_remote_attempts(
    *,
    ...
    for attempt in range(1, total_attempts + 1):
        ...
        # Pathé: never idle-only. AIMD can drop stack to 1; combined with busy GPUs
        # that starved submits ("idle-only · try 4/8") while active rows sat forever.
