    ...
    try:
        if backend == "runpod":
            ...
            try:
                ...
                push = push_clip_probe_to_pods(urls=bases, on_status=pod_status)
                if push.get("pushed"):
                    status(
                        f"CLIP probe on {push['pushed']} pod(s)",
                        job="pathe_scrape",
                        persist=True,
                    )
                elif push.get("error") == "probe_missing":
                    status(
                        "No local CLIP probe.pt — Pathé will use prompt scoring only",
                        job="pathe_scrape",
                        persist=True,
                    )
            except Exception as e:
                status(f"CLIP probe push skipped: {e}"[:160], job="pathe_scrape")
            status(
            ...
            # Soft ensure_pods already starts a background fill for the rest of the
            # fleet. Do not call ensure_pods(min_ready=n) again — that waits ~15 min
            # and leftover wait threads were stomping the live scrape status.
                    ...
                    try:
                        ...
                        if time.monotonic() - last_pool_sync >= 3.0:
                            ...
                            def _heal_status(msg: str) -> None:
                                # Console only — persisting every ~3s self-heal
                                # flooded SQLite and made /api/jobs look dead.
                        ...
                        print(f"[shtetl] pathe pool sync: {e}"[:200], flush=True)
                        try:
                            status(
                                f"GPU pool sync error: {e}"[:160],
                                job="pathe_scrape",
                                persist=True,
                            )
                        except Exception:
