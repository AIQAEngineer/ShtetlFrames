"""Rescore human Pass (decision=reject) stills; report any that still clear CLIP/OpenAI."""
...
OUT = OUTPUT_DIR / "pass_rescore_report.json"
...
def main() -> int:
    ...
    with db() as conn:
        rows = [
            ...
            for r in conn.execute(
                ...
                "FROM candidates WHERE decision='reject' ORDER BY id ASC"
            ).fetchall()
        ]
    print(f"pass_count={len(rows)} thr={DEFAULT_SCORE_THRESHOLD}", flush=True)
    if not rows:
        print("NO_PASSES", flush=True)
        OUT.write_text(json.dumps({"summary": {"n": 0}, "leaks": []}, indent=2))
        return 0
    ...
    for i, d in enumerate(rows, 1):
        ...
        if clip_pass:
            ...
            row["ok"] = False
            print(
                f"[{i}/{len(rows)}] #{cid} CLIP_LEAK score={score:.3f} cue={cue[:60]}",
                flush=True,
            )
        else:
            print(
        ...
        if do_openai and still_path is not None:
            try:
                ...
                if keep:
                    ...
                    row["ok"] = False
                    print(
                        f"  OPENAI_LEAK marker={row['openai_marker']} "
                        f"{row['openai_reason']}",
                        flush=True,
                    )
    ...
    summary = {
        "ts": int(time.time()),
        "n_pass_labels": len(rows),
        "n_scored": len(rows) - skip_n,
        "n_skip_no_still": skip_n,
        "n_still_ok": ok_n,
        "n_clip_leak": clip_leak_n,
        "n_openai_leak": openai_leak_n,
        "n_leaks": len(leaks),
        "score_threshold": thr,
        "all_pass_still_pass": len(leaks) == 0 and (len(rows) - skip_n) > 0,
    }
    OUT.write_text(
        json.dumps({"summary": summary, "leaks": leaks}, indent=2),
        encoding="utf-8",
    )
    print(json.dumps(summary), flush=True)
    print(f"wrote {OUT}", flush=True)
    return 0 if summary["all_pass_still_pass"] else 2
