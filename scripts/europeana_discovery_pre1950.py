"""Europeana discovery: all videos dated before 1950 across the network.
...
State (resumable):
  data/europeana/listing_pre1950.json       {record_id: record}
  data/europeana/cursor_pre1950.json        {range_label: nextCursor}
Output:
  output/europeana_discovery_pre1950.csv
...
CURSOR_JSON = STATE_DIR / "cursor_pre1950.json"
OUT_CSV = OUTPUT_DIR / "europeana_discovery_pre1950.csv"
...
def phase_listing() -> dict[str, dict]:
    ...
    def crawl_range(y0: int, y1: int) -> None:
        ...
        while not stop.is_set():
            ...
            if page % 5 == 0 or not items:
                ...
                log(f"[A] {label} p{page} union={len(records)}")
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        futs = [pool.submit(crawl_range, y0, y1) for y0, y1 in europeana.PRE1950_YEAR_RANGES]
        for f in as_completed(futs):
            f.result()
    _save_json(LISTING_JSON, records)
    log(f"[A] done: {len(records)} unique pre-1950 video records")
    return records
...
def print_stats(records: dict[str, dict]) -> None:
    ...
    for p, n in providers.most_common(15):
        print(f"  {n:6d}  {p}")
    print("\n== year buckets ==")
    for y0, y1 in europeana.PRE1950_YEAR_RANGES:
        n = sum(c for y, c in years.items() if y0 <= int(y) <= y1)
        print(f"  {y0}-{y1}: {n}")
    print(f"\ntotal pre-1950 video records: {len(records)}")
