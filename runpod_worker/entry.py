...
@app.post("/clip_probe")
def clip_probe(payload: dict) -> dict:
    """Install Keep/Pass CLIP linear probe weights (base64) without full sync_push."""
    import base64
    from pathlib import Path

    try:
        inp = dict(payload) if isinstance(payload, dict) else {}
        b64 = str(inp.get("b64") or inp.get("probe_b64") or "")
        if not b64:
            return {"ok": False, "error": "b64_required"}
        raw = base64.b64decode(b64)
        if len(raw) < 64:
            return {"ok": False, "error": "probe_too_small"}
...
def sync_push(payload: dict) -> dict:
    ...
    try:
        ...
        files_b64 = inp.get("files_b64") if isinstance(inp.get("files_b64"), dict) else {}
        ...
        for rel, b64 in files_b64.items():
            ...
            if ".." in name or name.startswith("/"):
                continue
            if name not in {"clip_ft/probe.pt"}:
                continue
            try:
        ...
        code_changed = [c for c in changed if c.endswith(".py")]
        reloaded = worker_sync._reload_modules(code_changed) if code_changed else []
        if "clip_ft/probe.pt" in changed:
            try:
                import handler as h

                if hasattr(h, "reset_models"):
                    h.reset_models()
                    reloaded = list(reloaded) + ["handler.reset_models"]
            except Exception:
                pass
