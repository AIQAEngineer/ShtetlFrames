...
def _work_slug(title: str, *, url: str = "", queue_id: Any = None) -> str:
    """Unique on-disk id so parallel Pathé jobs with the same title do not collide.

    Same-title assets used to share ``crops/<slug>/``; one job's cleanup deleted
    the other's JPEGs → ``[Errno 2] .../crops/selected_originals_royal_…``.
    """
    m = re.search(r"/asset/(\d+)", (url or ""), re.I)
    if m:
        return f"pathe_{m.group(1)}"
    base = slugify(title) or "video"
    if queue_id is not None and str(queue_id).strip() != "":
        return f"{base}_q{queue_id}"
    return base
